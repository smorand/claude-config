...
- **Referenced scripts**:
  - Use the **`run.sh` + `uv` pattern** for Python scripts (recommended for reliability and isolation)
  - If using the `run.sh` pattern, structure scripts as:
    ```
    scripts/
    ├── run.sh              # Generic script runner (takes script name as first arg)
    ├── pyproject.toml      # Python dependencies
    ├── .venv/              # Auto-created by uv
    └── src/
        └── script_name.py  # Python script
    ```
  - The `run.sh` script should use `uv run` to automatically create isolated environments and install dependencies
  - For non-Python scripts or simple cases, use standalone scripts
...
## References
- [Script Patterns](references/script-patterns.md) - run.sh + uv pattern with examples
