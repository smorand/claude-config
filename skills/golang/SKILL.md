...
```
name/
├── Makefile                  # Makefile to build quickly the project
...
└── src/                      # Python source code folder. MUST contain __init__.py for proper packaging
    ├── main.go               # Main package to start the implementation. Can reference the main sub-command
    ├── cli.go                # CLI implementation
    ├── api.go                # API implementation
    └── {functionality}.go    # One file per functionnality/object implementation
```

- Always put the entry point in main.go and use if for the main initization (logs)
